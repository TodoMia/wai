<template>
    <div>
        <div class="search">
            ...
            <div>
                <!-- <span class="chaxun" @click="upload">上传明文转密文</span> -->
                <span class="chaxun" @click="upload2">上传密文</span>
            </div>
        </div>
        <el-table v-loading="loading" :data="tableData" style="width: 100%; height: 100%">
            ...
            <el-table-column fixed="right" label="操作" width="200">
                <template slot-scope="scope">
                    <el-button @click="handleClick(scope.row)" :disabled="scope.row.status == 0" type="text" size="small">下载明文</el-button>
                    <!-- <el-button @click="handleClick2(scope.row)" :disabled="scope.row.status == 0" type="text" size="small">下载不可营销</el-button> -->
                    <!-- <el-tooltip class="item" effect="dark" content="下载明文" placement="top-start">
    ...
    methods: {
        chaxun(){
			...
			.then(res => {
                ...
                // console.log(res.data.filelist[0].filename);
                // console.log(res.data.filelist[0].id);
                // var { filename } = res.data.filelist[0]
                // console.log(res.data.filelist[0].batch⁯num);
            })
            .catch(err => {
                console.error(err);
            })
